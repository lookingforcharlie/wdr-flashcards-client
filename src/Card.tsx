...
function Card(Props: TProps) {
  ...
  const handleEditButton = (card: TCard) => {
    ...
    // find the target card by it's index
    const findCard = cards.find((item) => item._id === card._id);
    console.log('Found Card: ', findCard);

    // move the targets of modification back to input erea
    if (findCard) {
  ...
  return (
    <div
      onClick={handleFlip}
      className='relative [perspective:1000px]  bg-gray-300 shadow-lg border border-stone-800 rounded-md p-12  hover:bg-gray-200 cursor-pointer'
    >
